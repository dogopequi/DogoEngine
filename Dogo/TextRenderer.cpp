...
namespace Dogo
{
	void TextRenderer::Init()
	{
		...
		{
			DG_FATAL("ERROR::FREETYPE: Could not init FreeType Library");
			return;
		}
		...
		if (font_name.empty())
		{
			DG_FATAL("ERROR::FREETYPE: Failed to load font_name");
			return;
		}
		...
		if (FT_New_Face(ft, font_name.c_str(), 0, &face)) {
			DG_FATAL("ERROR::FREETYPE: Failed to load font");
			return;
		}
		else {
			...
			for (unsigned char c = 0; c < 128; c++)
			{
				...
				{
					DG_FATAL("ERROR::FREETYTPE: Failed to load Glyph");
					continue;
				}
				// generate texture
